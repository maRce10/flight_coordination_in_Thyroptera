mod_xcorr <- function (X = NULL, wl = 512, bp = "pairwise.freq.range", ovlp = 70, 
          ...
          method = 1) 
{
  ...
  if (length(opt.argms) > 0) 
    for (q in 1:length(opt.argms)) assign(names(opt.argms)[q], 
                                          opt.argms[[q]])
  if (is.null(path)) 
    path <- getwd() else if (!dir.exists(path) & !is_extended_selection_table(X)) 
    stop("'path' provided does not exist") else path <- normalizePath(path)
  ...
  if (!is.numeric(ovlp)) 
    stop("'ovlp' must be a numeric vector of length 1") else {
    if (!is.vector(ovlp)) 
      stop("'ovlp' must be a numeric vector of length 1") else {
      if (!length(ovlp) == 1) 
    ...
    if (pb) 
      if (type == "spectrogram") 
        write(file = "", x = paste0("creating spectrogram matrices (step 1 of ", 
                                    max.stps, "):")) else write(file = "", x = paste0("creating MFCC matrices (step 1 of ", 
                                     max.stps, "):"))
    pbapply::pboptions(type = ifelse(pb, "timer", "none"))
    if (Sys.info()[1] == "Windows" & parallel > 1) 
      cl <- parallel::makePSOCKcluster(getOption("cl.cores", 
                                                 parallel)) else cl <- parallel
    spcs <- pbapply::pblapply(X = 1:nrow(X), cl = cl, function(e) spc_FUN(j = e, 
                                                                          pth = path, W = X, wlg = wl, ovl = ovlp, w = wn, 
                                                                          nbnds = nbands))
  ...
  xcrrs <- pbapply::pblapply(X = 1:nrow(spc.cmbs), cl = cl, 
                             FUN = function(j, BP = bp, cor.meth = cor.method) {
                               ...
                               if (method == 2) {
                                 spc1 <- spc_FUN(j = which(X$selection.id == 
                                                             spc.cmbs[j, 1]), pth = path, W = X, wlg = wl, 
                                                 ovl = ovlp, w = wn, nbnds = nbands)
                                 spc2 <- spc_FUN(j = which(X$selection.id == 
                                                             spc.cmbs[j, 2]), pth = path, W = X, wlg = wl, 
                                                 ovl = ovlp, w = wn, nbnds = nbands)
                               }
  ...
  if (output == "cor.mat") 
    ...
    output_list <- list(max.xcorr.matrix = mat, scores = cor.table, 
                        selection.table = X, 
                        hop.size.ms = read_wave(X, 1, header = TRUE, path = path)$sample.rate/wl, 
                        errors = errors)
