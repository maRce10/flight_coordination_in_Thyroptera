...
warbleR_options(wav.path = "~/Dropbox/Recordings/flight_coordination_Thyroptera/converted_sound_files_90_kHz/", wl = 300, parallel = parallel::detectCores() - 1, bp = "frange", fast = F, threshold = 15, ovlp = 20)
...
templ$selec <- 10001
split_sels_temp <- rbind(templ, split_sels)

comp_mat <- cbind(paste(split_sels_temp$sound.files[1], split_sels_temp$selec[1], sep = "-"), split_sels$sound.files)
...
{
  xc.output <- xcorr(X = split_sels_temp, output = "list", 
...
# get in whole file clips
split_sels <- split_wavs(only.sels = TRUE, sgmt.dur = 200000, files = not_done)
...
# templ2$bottom.freq[1] <- 13.3548
templ$selec <- 10001
split_sels <- split_sels[split_sels$sound.files != "ch4MPI2020-01-19_15-03-57_0000057.wav", ]
